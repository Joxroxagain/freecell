...
public class Tester {
	...
	public static void main(String[] args) {
		...
		if (tableaux[1].canRemoveFrom(tableaux[1].getTopCard()) && tableaux[2].canAddTo(tableaux[1].getTopCard())) {
			System.out.println(
					"Can move " + tableaux[1].getTopCard() + " to tableau 2 which has a " + tableaux[2].getTopCard());
		} else {
			System.out.println("Can NOT move " + tableaux[1].getTopCard() + " to tableau 2 which has a "
					+ tableaux[2].getTopCard());
		}

		// Blindly test home cells by trying to move the top card from
		// tableau 1 to home cell 1
		if (tableaux[1].canRemoveFrom(tableaux[1].getTopCard()) && homeCells[1].canAddTo(tableaux[1].getTopCard())) {
			System.out.println("Can move " + tableaux[1].getTopCard() + " to home cell 1 because its an ace!");
		} else {
			System.out
					.println("Can NOT move " + tableaux[1].getTopCard() + " to home cell 1 because it is not an ace!");
		}
